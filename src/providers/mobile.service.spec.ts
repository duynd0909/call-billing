...
describe('MobileService', () => {
  ...
  describe('createCall', () => {
    it('should create a new mobile call', async () => {
      const createCallRequest: CreateCallRequest = {
        username: MOBILE_USERNAME,
        call_duration: 30000,
      };

      const mockMobileCall = new Mobile();
      mockMobileCall.username = MOBILE_USERNAME;
      mockMobileCall.duration = 30000;
      mockMobileCall.createdAt = new Date();
      jest.spyOn(mobileRepository, 'save').mockResolvedValueOnce({
        ...mockMobileCall,
      });

      const mobileCallResult = await mobileService.createCall(
        createCallRequest.username,
        createCallRequest,
      );
      expect(mobileCallResult.duration).toEqual(mockMobileCall.duration);
    });
